...
public abstract class AbstractGenericConverter extends UltraBasicGenericMapper {
	...
	public abstract Object getSpecificConverter();
	...
	try {
		...
		MethodHandle handle = MethodHandles.publicLookup().findVirtual(getSpecificConverter().getClass(), convertMethodName, convertMethType);
		
		Function<Object,Object> transformFunction =(entity)->{
			try {
				return handle.invoke(getSpecificConverter(), entity);
			} catch (Throwable e) {
				e.printStackTrace();
				return null;
			}
